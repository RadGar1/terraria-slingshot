...
{
    public class shinyRock : ModProjectile
    {
        ...
        public override bool PreKill(int timeLeft) //for some reason does not spawn shards if there are still shards from the last rock
        {
            ...
            Vector2 launchVelocity = new Vector2(-3, 0);
            for (int i = 0; i < 4; i++)
            {
                launchVelocity = launchVelocity.RotatedBy(MathHelper.PiOver4);
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, launchVelocity, ModContent.ProjectileType<pearlShard>(), Projectile.damage / 3, Projectile.knockBack / 4, Projectile.owner);
            }
